...
<p class=\"MsoNormal\">SeilConnect: <o:p></o:p></p>

<p class=\"MsoNormal\">Die Connectoren SeilConnect bilden die Schnittstelle
zwischen dem Antrieb des Motors und den Rollen bzw. der Masse des
Flaschenzuges. Dabei wird der zurückgelegte Weg des Seiles sowie die benötigte
Kraft übergeben, welche notwendig ist, um die Masse anzuheben. Außerdem erfolgt
die Übergabe der Gesamtanzahl der Rollen.<o:p></o:p></p>

<p class=\"MsoNormal\">Anwendung: <o:p></o:p></p>

<p class=\"MsoNormal\">Die Connectoren sind bereits an den jeweiligen Bauteilen
angebracht, sodass diese nicht händisch implementiert werden müssen<o:p></o:p></p>
...
<p class=\"MsoNormal\">Spannungsport: <o:p></o:p></p>

<p class=\"MsoNormal\">Der Spannungsport bildet die Schnittstelle zwischen der
Spannungsquelle und dem Motor. Hierbei wird lediglich die am Motor anliegende
Gleichspannung übertragen. <o:p></o:p></p>
...
<!--EndFragment--></body></html>"));
    ...
    connector SeilPort
      ...
      Modelica.SIunits.Length h;
      Modelica.SIunits.Velocity v_Masse;
      Modelica.SIunits.Velocity v_Seiltrommel;
      annotation(
...
<p class=\"MsoNormal\">SeilPort: <o:p></o:p></p>

<p class=\"MsoNormal\">Der Connector SeilPort bildet die Schnittstelle zwischen dem
Motor und der Seiltrommel. Hierbei werden u.A die Geschwindigkeit des Motors
sowie das Lastmoment der Masse übergeben. <o:p></o:p></p>
...
<p class=\"MsoNormal\">ScopePort: <o:p></o:p></p>

<p class=\"MsoNormal\">Der ScopePort bildet die Schnittstelle zwischen der
Antriebskomponente und dem Scope. Hierbei werden alle relevanten Größen des
Systems übergeben, welche im Scope betrachtet werden können. <o:p></o:p></p>
...
// Weitergabe an den Connector in Richtung Antrieb
      seilConnect1.h = h + h0;
      seilConnect1.Fg = Fg;
      seilConnect1.h_max = h_max*abs(seilConnect1.nGes);
// Weitergabe an den Connector Richtung Scope
    ...
    model Scope "Anzeige ausgewählter Daten"
      ...
      input Modelica.SIunits.Velocity v_Seiltrommel "Geschwindigkeit Seiltrommel";
    ...
    equation
      ...
      scopePort1.s = s;
      scopePort1.h = h;
      scopePort1.v_Seiltrommel = v_Seiltrommel;
    ...
    package Antrieb
      ...
      //Spannungen und Ströme
        ...
        Modelica.SIunits.Length s_tr "Seilweg";
        Modelica.SIunits.Length s_max_real "Seilweg";
        Modelica.SIunits.Velocity v_Seiltrommel "Geschwindigkeit Seiltrommel"; 
...
// Berechnungen von Drehgeschewindigkeit und Seilgeschwindigkeit
        ...
        der(s_tr) = v_Motor;
        s = Faktor_s * (s_max_real) * (1 - exp(-abs(b*s_tr)));
        s_max_real = if seilConnect1.h_max < s_max then seilConnect1.h_max else s_max;
...
// Zusammenhang zwischen Drehmoment und Geschwindigkeit
        M_eff = M_E - M_L;
        v_Seiltrommel = seilConnect1.v_Masse*abs(seilConnect1.Fg/seilConnect1.Fz);
...
// Werteübergabe an das Scope
          ...
          scopePort1.v_Seiltrommel = seilPort1.v_Seiltrommel;
...
<!--EndFragment--></body></html>"));
        ...
        // Variablen, um Daten von der Masse über den Motor an das Scope weiterzuleiten
          Modelica.SIunits.Length h "Hubhöhe der Masse";
          Modelica.SIunits.Length s_max_real "Maximaler Seilweg";
          Modelica.SIunits.Force Fg "Gewichtskraft der angehängten Masse";
          Modelica.SIunits.Velocity v_Masse "Hubgeschwindigkeit der Masse";
          Modelica.SIunits.Velocity v_Seiltrommel "Geschwindigkeit Seiltrommel";
...
// Allgemeine Gleichungen
          ...
          der(s_tr) = v_Motor;
          s = seilPort1.Faktor_s * (s_max_real) * (1 - exp(-abs(seilPort1.b * s_tr)));
          s_max_real = if seilConnect1.h_max < s_max then seilConnect1.h_max else s_max;
          seilPort1.M_L = F_za * r;
          v_Seiltrommel = seilPort1.v_Seiltrommel;
          F_za = -seilPort1.Fz;
          v_Seiltrommel = v_Masse*abs(Fg/F_za);
...
//Weitergabe an den Connector
    ...
    model Anschlussarten "Mögliche Arten des Anschlusses"
      model Invertierter_Anschluss "Invertierter Anschluss"
        ...
        parameter Integer n = 1 "Anzahl der Rollen" annotation(
          Dialog(tab = "Rollen"));
        parameter Real eta = 100 "Wirkungsgrad der Rollen in %" annotation(
          Dialog(tab = "Rollen"));
      // Konstanten
        constant Modelica.SIunits.Acceleration g_n = Modelica.Constants.g_n "Erdbeschleunigung";
      // Masse und Gewichtskraft
      ...
      //Hubhöhe und Raum
        parameter Modelica.SIunits.Length h_max = 10 "Maximale Hubhöhe" annotation(
          Dialog(tab = "Raum"));
        parameter Modelica.SIunits.Length h0 = 0 "Starthöhe" annotation(
          Dialog(tab = "Raum"));
        Modelica.SIunits.Length h "Hubhöhe der Masse";
      ...
      /* Berechnung von Seilkraft und Hubhöhe;
        ...
        h = (seilConnect1.s / (abs(seilConnect1.nGes) + n + 1))+h0;
...
//Berechnung von Hubgeschwindigkeit und -beschleunigung
        v_Masse = der(h);
        a = der(v_Masse);
        seilConnect1.h_max = h_max*(abs(seilConnect1.nGes) + n + 1);
...
// Weitergabe von Variablen zwischen Seiltrommel und Masse
        ...
        seilConnect1.h_max = seilConnect2.h_max;
