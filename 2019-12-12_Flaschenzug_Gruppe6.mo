  ...
  model Flaschenzug_Komponenten
    ...
    package Antrieb
      model Antrieb "Elektrischer Gleichstrommaschine"
        ...
        Antrieb_Komponenten.Spannungsquelle s_quelle(U=U);
  ...
  Documentation(info = "<html><head></head><body><b>Beschreibung:&nbsp;</b><div><br></div><div>- Hier werden die Komponenten Spannungsquelle, Elektromotor und Seiltrommel in einem Package zur Verfügung gestellt</div><div><br></div><div>-Für weitere Informationen bitte in die Dokumentation der jewiligen Modelle schauen</div><div><br></div><div><b>Anwendung:</b></div><div><br></div><div>Der Aufbau besitzt eine Schnittstelle in Form eines Connectors, an die eine Rolle oder direkt die Masse gehängt werden kann. Ansonsten sind die Parameter durch doppelklicken der Anwendung einstellbar</div></body></html>"));
      ...
      class Antrieb_Komponenten
        model Motor
          ...
          parameter Real b = 0.002 "Anlaufkoeffient";
...
//Berechnung des Moments
          ...
          M_E = kt * I*((-abs(w) / w_max) + 1);//M_E mit geschwindigkeitsabhängigem Wirkungsgrad versehen
          M_L = M_E*(1-abs(d))+seilPort1.M_L*abs(d); //Wenn dir Bremse aktiv ist gilt M_L = M_E wenn nicht gilt das M_L von der Seiltrommel
          M_E - M_L = (Jgm + M_L * seilPort1.r) * der(w); //ZUsammenhang zwischen Drehmoment und Geschwindigkeit
          M_eff = M_E - M_L;
...
<!--EndFragment--></body></html>"));
  ...
  Documentation(info = "<html><head></head><body><b>Beschreibung:&nbsp;</b><div><b><br></b><div>- Einstellung der anliegenden Spannung am Motor per Parameter <i>U</i></div><div><i><br></i></div><div><span style=\"font-size: 12px;\"><b>Anwendung:</b>&nbsp;</span></div><div><span style=\"font-size: 12px;\"><br></span></div><div><span style=\"font-size: 12px;\">Die Spanungsquelle kann auch einzeln bei Drag-and-Drop in das Modell implementiert werden und mit einem einzelnen Motor verbunden werden. Empfohlen wird jedoch die Antriebskomponente zu verwenden, in der die Spannungsquelle mit dem Motor und der Seiltrommel in einem Package vereint ist.&nbsp;</span></div><div><br></div></div></body></html>"));
    ...
    model Anschlussarten
      ...
      equation
        ...
        seilConnect1.Fz * (abs(seilConnect1.nGes) + 1 + n) * (eta / 100) = m * g_n;
        Fg = m * g_n;
  ...
  package Modelle
    model Bsp_Einfache_Rolle_Parameter
    ...
    equation
      ...
      annotation(
        ...
        Diagram,
        __OpenModelica_commandLineOptions = "");
    end Bsp_Einfache_Rolle_Parameter;
 
    model Bsp_Invertierter_Anschluss
    ...
    equation
      ...
      annotation(
        ...
        Diagram,
        __OpenModelica_commandLineOptions = "");
    end Bsp_Invertierter_Anschluss;
 
    model Bsp_Kombination_Einfache_Rolle_Invertierter_Anschluss
      Flaschenzug_Gruppe6.Flaschenzug_Komponenten.Anschlussarten.Invertierter_Anschluss invertierter_Anschluss1 annotation(
    ...
    equation
      ...
      annotation(
        ...
        Diagram,
        __OpenModelica_commandLineOptions = "");
    end Bsp_Kombination_Einfache_Rolle_Invertierter_Anschluss;
 
    model Bsp_Einfache_Rolle_DragAndDrop
    Flaschenzug_Gruppe6.Flaschenzug_Komponenten.Masse masse1 annotation(
  ...
  annotation(
        ...
        Diagram,
        __OpenModelica_commandLineOptions = "");
    end Bsp_Einfache_Rolle_DragAndDrop;
 
    model Bsp_Ohne_Rollen
