package Flaschenzug_Gruppe6 "Modell zur Erstellung eines Flaschenzuges im Rahmen des Masterstudium Mechatronische Systementwicklung an der Hochschule Pforzheim. "
  ...
  class UsersGuide "User's Guide"
   ...
   annotation (Documentation(info = "<html><head></head><body><p><font size=\"5\">
    ...
    package Antrieb
      model Antrieb "Antrieb"
       ...
       parameter Modelica.SIunits.Voltage U = 24 "Versorgungsspannung" annotation(
        ...
        Modelica.SIunits.Velocity v_Motor "Geschwindigkeit des Motors";
      ...
      //M_E mit geschwindigkeitsabhängigem Wirkungsgrad versehen
        M_L = M_E * (1 - abs(d)) + F_za * r * abs(d);
      //Wenn dir Bremse aktiv ist gilt M_L = M_E wenn nicht gilt das M_L von der Seiltrommel
        M_E - M_L = (Jgm + M_L * r) * der(w);
      //ZUsammenhang zwischen Drehmoment und Geschwindigkeit
        M_eff = M_E - M_L;
      // Werteübergabe an diverse Connectoren
        
       
        M_E = scopePort1.M_E;
        M_L = scopePort1.M_L;
        F_za = scopePort1.Fz;
        seilConnect1.Fg = scopePort1.Fg;
        s = scopePort1.s;
        seilConnect1.h = scopePort1.h;
        v_Motor = scopePort1.v_Motor;
        seilConnect1.v_Masse = scopePort1.v_Masse;
...
//Berechnung des Moments
          M_E = kt * I * ((-abs(w) / w_max) + 1);
//M_E mit geschwindigkeitsabhängigem Wirkungsgrad versehen
          M_L = M_E * (1 - abs(d)) + seilPort1.M_L * abs(d);
//Wenn dir Bremse aktiv ist gilt M_L = M_E wenn nicht gilt das M_L von der Seiltrommel
...
//Allgemeine Gleichungen
          ...
          der(s_tr) = v_Motor;
          s = seilPort1.d * (-s_max) * (1 - exp(-abs(seilPort1.b*s_tr)));
...
<!--EndFragment--></body></html>"));
  ...
  Documentation(info = "<html><head></head><body><b>Beschreibung:&nbsp;</b><div><b><br></b><div>- Einstellung der anliegenden Spannung am Motor per Parameter <i>U</i></div><div><i><br></i></div><div><span style=\"font-size: 12px;\"><b>Anwendung:</b>&nbsp;</span></div><div><span style=\"font-size: 12px;\"><br></span></div><div><span style=\"font-size: 12px;\">Die Spanungsquelle kann auch einzeln bei Drag-and-Drop in das Modell implementiert werden und mit einem einzelnen Motor verbunden werden. Empfohlen wird jedoch die Antriebskomponente zu verwenden, in der die Spannungsquelle mit dem Motor und der Seiltrommel in einem Package vereint ist.&nbsp;</span></div><div><br></div></div></body></html>"));
    ...
    model Anschlussarten "Mögliche Arten des Anschlusses"
      ...
      equation
        ...
        seilConnect1.Fz * (abs(seilConnect1.nGes) + n) * (eta / 100) = Fgm;
        Fgm = abs(m * a + Fg);
        Fg = m * g_n;
        h = seilConnect1.s / (abs(seilConnect1.nGes) + n);
        v_Masse = der(h);
        a = der(v_Masse);
      ...
      equation
       ...
       //seilConnect2.nGes = seilConnect1.nGes + n;
 ...
 Flaschenzug_Komponenten.Antrieb.Antrieb_Komponenten.Spannungsquelle spannungsquelle1 annotation(
        Placement(visible = true, transformation(origin = {-88, -20}, extent = {{-8, -7.3}, {8, 7.3}}, rotation = 0)));
 Flaschenzug_Komponenten.Antrieb.Antrieb_Komponenten.Seiltrommel seiltrommel1 annotation(
        Placement(visible = true, transformation(origin = {18, -16}, extent = {{-8.5, -8.5}, {8.5, 7.7}}, rotation = 0)));
 Flaschenzug_Gruppe6.Flaschenzug_Komponenten.Antrieb.Antrieb_Komponenten.Motor motor1 annotation(
        Placement(visible = true, transformation(origin = {-50, -16}, extent = {{-8.3, -4.5}, {8.3, 4.9}}, rotation = 0)));
    equation
      connect(seiltrommel1.seilConnect1, rolle1.seilConnect1) annotation(
        Line(points = {{22, -24}, {18, -24}, {18, 40}, {18, 40}}));
      connect(motor1.scopePort1, scope1.scopePort1) annotation(
        Line(points = {{-56, -12}, {-82, -12}, {-82, 30}, {-82, 30}}));
      connect(motor1.seilPort1, seiltrommel1.seilPort1) annotation(
        Line(points = {{-42, -14}, {10, -14}, {10, -16}, {10, -16}}));
      connect(spannungsquelle1.SpannungsPort1, motor1.SpannungsPort1) annotation(
        Line(points = {{-82, -20}, {-56, -20}, {-56, -18}, {-56, -18}}));
 ...
 Flaschenzug_Komponenten.Masse masse1 annotation(
